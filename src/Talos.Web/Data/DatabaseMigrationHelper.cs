...
public static class DatabaseMigrationHelper
{
    ...
    /// creates the history table and records all known migrations so that
    ...
    private static async Task SeedMigrationHistoryIfNeededAsync(TalosDbContext db)
    {
        ...
        try
        {
            if (await MigrationHistoryExistsAsync(conn))
                return;

            if (!await ApplicationTablesExistAsync(conn))
                return;

            await CreateAndSeedHistoryTableAsync(conn);
        }
        finally
        {
    ...
    private static async Task CreateAndSeedHistoryTableAsync(System.Data.Common.DbConnection conn)
    {
        await using var cmd = conn.CreateCommand();
        cmd.CommandText = """
